...
from estimator import LagGPTEstimator, LagGPTEstimatorWithoutTimeFeatures
import os
...
def train(args):
    ...
    fulldir = os.path.join(pathlib.Path(__file__).parent.resolve(), "model-size-scaling-logs", str(args.seed)) # Always creates the experiment directory inside "lag-gpt"
    ...
    # else:
    #     print(f"No checkpoint found at {logger.log_dir}/checkpoints. Training from scratch.")
    #     ckpt_path = None
    ...
    estimator = LagGPTEstimatorWithoutTimeFeatures(
        prediction_length=1,
