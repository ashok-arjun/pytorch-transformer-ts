...
from lightning_module import LagGPTLightningModule, LagGPTLightningModuleWithoutTimeFeatures
...
PREDICTION_INPUT_NAMES_NO_TIME_FEATURES = [
    "past_target",
    "past_observed_values",
]

TRAINING_INPUT_NAMES_NO_TIME_FEATURES = PREDICTION_INPUT_NAMES_NO_TIME_FEATURES + [
    "future_target",
    "future_observed_values",
]
class LagGPTEstimatorWithoutTimeFeatures(PyTorchLightningEstimator):
    """
    An estimator training a ConvTSMixer model for forecasting.
    ...
    def create_lightning_module(self) -> pl.LightningModule:
        ...
        if self.ckpt_path is not None:
            return LagGPTLightningModuleWithoutTimeFeatures.load_from_checkpoint(
                checkpoint_path=self.ckpt_path,
                loss=self.loss,
                lr=self.lr,
                weight_decay=self.weight_decay,
            ...
            return LagGPTLightningModuleWithoutTimeFeatures(
                loss=self.loss,
                lr=self.lr,
                weight_decay=self.weight_decay,
                context_length=self.context_length,
    ...
    def _create_instance_splitter(self, module: LagGPTLightningModuleWithoutTimeFeatures, mode: str):
        assert mode in ["training", "validation", "test"]
    ...
    def create_training_data_loader(
        self,
        data: Dataset,
        module: LagGPTLightningModuleWithoutTimeFeatures,
        shuffle_buffer_length: Optional[int] = None,
        **kwargs,
    ) -> Iterable:
        data = Cyclic(data).stream()
        ...
        return as_stacked_batches(
            ...
            batch_size=self.batch_size,
            shuffle_buffer_length=shuffle_buffer_length,
            field_names=TRAINING_INPUT_NAMES_NO_TIME_FEATURES,
            output_type=torch.tensor,
            num_batches_per_epoch=self.num_batches_per_epoch,
        )
    ...
    def create_predictor(
        ...
        return PyTorchPredictor(
            input_transform=transformation + prediction_splitter,
            input_names=PREDICTION_INPUT_NAMES_NO_TIME_FEATURES,
            prediction_net=module,
            batch_size=self.batch_size,
            prediction_length=self.prediction_length,
            device="cuda" if torch.cuda.is_available() else "cpu",
